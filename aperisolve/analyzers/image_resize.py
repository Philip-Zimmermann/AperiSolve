...
def analyze_image_resize(input_img: Path, output_dir: Path) -> None:
    ...
    try:
        ...
        # --- STRATEGY 2: Check SQLite DB (Fallback) ---
        if not match_found:
        ...
        if match_found and candidates:
            ...
            output_path = output_dir / img_name  # Direct save to output_dir

            # Create parent directories if they don't exist (safety)
            output_dir.mkdir(parents=True, exist_ok=True)

            write_recovered_png(b, ihdr, w, h, output_path)

            # Construct path for frontend
        ...
        # Add the 'image' key if we found one
        if recovered_image:
